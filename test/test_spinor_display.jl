...
@testset "Spinor display: _spinor_base_name edge cases" begin
    ...
    # Extended names (from fresh_spinor_index overflow)
    @test TensorGR._spinor_base_name(:Ap1) == "Ap1"  # only strip trailing 'p', not mid-string
    # Note: extended dotted names like :Ap1 don't end in 'p', so they pass through.
    # The name :Ap1p (if it existed) would strip to "Ap1"
end
