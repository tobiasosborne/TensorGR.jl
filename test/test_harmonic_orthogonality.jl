...
@testset "Vector harmonic orthogonality" begin
    ...
    # ── Odd-odd orthogonality (MP Eq 2.14) ───────────────────────────────
    # integral X_bar^A_{lm} X_A^{l'm'} dOmega = l(l+1) delta_{ll'} delta_{mm'}
    @testset "Odd-odd: MP Eq 2.14" begin
        # l=2, m=1: l(l+1) = 6
    ...
    # ── Even-odd cross-orthogonality (MP Eq 2.15) ────────────────────────
    # integral Y_bar^A_{lm} X_A^{l'm'} dOmega = 0 for all l,m,l',m'
    @testset "Even-odd cross: MP Eq 2.15" begin
        @test inner_product(EvenVectorHarmonic(2, 1, up(:a)),
                            OddVectorHarmonic(2, 1, up(:a))) == TScalar(0)
        @test inner_product(OddVectorHarmonic(2, 1, up(:a)),
    ...
    # ── Exhaustive check l=1..5 (MP Eqs 2.13-2.15) ──────────────────────
    @testset "Exhaustive l=1..5: MP Eqs 2.13-2.15" begin
    ...
    # ── Scalar harmonic orthogonality (MP Eq 2.3, verify existing) ───────
    @testset "Scalar: MP Eq 2.3" begin
